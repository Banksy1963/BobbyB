# magellan_hybrid.py — Final Hybrid Bot with GUI, CLI, Auto-Scan, Log Viewer, Drawdown Guard
...
class PortfolioManager:
    ...
    def check_drawdown(self, current_value):
        ...
        drawdown = (self.peak_value - current_value) / self.peak_value if self.peak_value else 0
        return drawdown < 0.18  # Limit drawdown to 18%
...
class OrderExecutor:
    ...
    for ticker in tickers:
        try:
            ...
            # Check drawdown before placing trade
            portfolio_value = sum([get_live_price(t) * p['shares'] for t, p in pm.portfolio.items()]) if pm.portfolio else 0
            if not pm.check_drawdown(portfolio_value):
                print(f"🚫 Drawdown exceeds 18%. Skipping {ticker}.")
                continue
...
def launch_gui():
    ...
    tk.Radiobutton(root, text="Paper Mode", variable=mode_var, value="paper").pack()
    tk.Radiobutton(root, text="Live Mode", variable=mode_var, value="live").pack()

    auto_var = tk.BooleanVar()
    tk.Checkbutton(root, text="Auto-Scan Momentum Tickers", variable=auto_var).pack()

    def show_logs():
        try:
            with open('stock_stories.txt', 'r') as f:
                logs = f.read()
        except FileNotFoundError:
            logs = "No logs found."

        log_window = tk.Toplevel()
        log_window.title("Trade Logs")
        text_widget = tk.Text(log_window, wrap='word', width=80, height=20)
        text_widget.insert('1.0', logs)
        text_widget.pack()

    def on_run():
        if auto_var.get():
            tickers = scan_for_momentum(['AAPL','TSLA','NVDA','AMD','PLTR','GOOG','META'])
        else:
            tickers = ticker_entry.get().split(',')
        run_bot(tickers)
        messagebox.showinfo("Bot Status", "Bot executed successfully.")

    tk.Button(root, text="Run Bot", command=on_run).pack()
    tk.Button(root, text="View Logs", command=show_logs).pack()
    root.mainloop()
